...
export function query(path, funcs, options) {
  ...
  // rather than overflowing the native stack. An explicit Infinity normalizes
  // to the undefined slot `limit()` already reads as unbounded.
  const limits = [opts.maxNodes, opts.maxDepth ?? 500, opts.maxResults].map((v) =>
