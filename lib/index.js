 ...
 */
let utf16Pair = (raw, j, high, low = hex4(raw, j + 3)) =>
  // oxlint-disable-next-line no-unused-expressions
  (
    +(raw.slice(j + 1, j + 3) === "\\u") & +((low & 0xfc00) === 0xdc00) || bad(),
 ...
 * @returns {[string, number]}
 */
let unescapeU = (raw, j, a = hex4(raw, j + 1)) => {
  // oxlint-disable-next-line no-unused-expressions
  a >= 0 || bad();
  if ((a & 0xfc00) === 0xd800) {
    const pair = utf16Pair(raw, j + 4, a);
 ...
 * @returns {[string, number]}
 */
let unescape = (raw, j, q, ch = ("/\\" + q).includes(raw[j]) ? raw[j] : ESC[raw[j]]) => {
  if (ch) return [ch, 0];
  if (raw[j] === "u") return unescapeU(raw, j);
  bad();
};
/**
 * @param {string} raw
 ...
 * @returns {[string, number]}
 */
let takeScalar = (raw, j, c, a = c.charCodeAt(0)) => {
  if ((a & 0xfc00) === 0xd800) {
    // oxlint-disable-next-line no-unused-expressions
    (raw.charCodeAt(j + 1) & 0xfc00) === 0xdc00 || bad();
    return [c + raw[j + 1], 1];
  }
  // oxlint-disable-next-line no-unused-expressions
  (a & 0xfc00) !== 0xdc00 || bad();
...
let stepUnq = (raw, j, q) => {
  if (raw[j] === "\\") {
    const got = unescape(raw, j + 1, q);
    return [got[0], j + 2 + got[1]];
  }
  const c = raw[j];
  // oxlint-disable-next-line no-unused-expressions
  (c >= " " && c !== q) || bad();
  const got = takeScalar(raw, j, c);
 ...
 * @returns {string}
 */
let unq = (raw, q = raw[0], end = raw.length - 1) => {
  let out = "",
    j = 1;
  while (j < end) {
    const step = stepUnq(raw, j, q);
    out += step[0];
    j = step[1];
  }
  return out;
...
/**
 * @param {[string | 0, number, number, string?, boolean?]} step
 * @param {number} from
 */
...
let close = (text, from) => {
  ...
  for (; from < text.length; from++) {
    const step = scanBracketChar(text[from], q, d);
    q = step[0];
    d = step[1];
    from += step[2];
    if (+!step[3] | step[1]) continue;
    return step[3] === "]" ? from : err("Missing ] in path");
  }
  err("Missing ] in path");
};
 ...
 * @returns {Sel}
 */
let nameOrIndex = (spec, quoted = /^(['"])([\s\S]*)\1$/.test(spec)) => {
  if (quoted) {
    const key = unq(spec);
    return {
...
let selector = (
  ...
  if (spec[0] === "?") {
    const test = rfcFilter(spec.slice(1), registry, info);
    return {
 ...
 * @returns {number}
 */
let skip = (s, i, m = /^\s*/.exec(s.slice(i))) => i + (m ? m[0].length : 0);
/**
 * @param {string} path
 * @param {number} j
...
let unexpectedSeg = (path, j, back, soft, segs) =>
  // oxlint-disable-next-line no-unused-expressions
  (soft || err('Unexpected "' + path[j] + '" in path'), { segs, j: back });
/**
 * @param {string} path
 * @param {number} j
 * @param {boolean} desc
...
let bracketSeg = (path, j, desc, registry, info, dep) => {
  ...
  const sels = raw.map((s) => selector(s, registry, info));
 ...
 */
let userArgs = (name, f = fns[name]) => {
  // oxlint-disable-next-line no-unused-expressions
  Object.hasOwn(fns, name) || err(name + " is not a function");
  // oxlint-disable-next-line no-unused-expressions
 ...
 * @returns {Test}
 */
let rfcFilter = (src, registry, info, prev = [source, k, fns, meta]) => {
  source = src;
  k = 0;
  fns = registry;
  meta = info;
