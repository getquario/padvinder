 ...
 *
 * Deliberately `any`: a precise tuple type buys a cast at every access on the
 * hottest path in the engine, and no real safety.
 *
 * @internal
 * @typedef {any} Ctx
 */
...
/**
 * A parsed filter primary: `t` is 0 for a value, 1 for a query (whose `f` is
 * a {@link Query}, not an `Arg`), 2 for a logical function, 3 for one from the
 * caller's registry. Every position demands against that type.
 *
 * @internal
 ...
 * text the caller is parsing; `base` carries them to the coordinates of the
 * string `query()` was handed, so a nested parser need not know its depth.
 *
 * `code` defaults to the open-ended residue: the two path faults and the
 * filter parser's catch-all name no single mistake. The five that do say so.
 *
 ...
 * whatever text the caller is parsing, so the span is the offending escape
 * rather than the whole literal. One code covers the category; `m` is what
 * tells the seven conditions apart, for an author to read.
 *
...
let kids = (
  ...
  else if (isObj(value)) nameKids(out, value, depth, ctx);
  return out;
};
...
/**
 * The surrogate rule both string readers obey: a high half must be followed by
 * a low one, and a lone low half is not a Unicode scalar value. Either fault
 * is the first half's, so both span `[from, to)`; an absent `low` arrives
 * as -1. `noun` is what the two readers disagree about, for the message.
 *
...
/**
 * Throw for a character that cannot stand unescaped in a literal. `stepUnq`
 * gives its two faults one test and one span, so only the message tells them
 * apart, and the reader keeps its branchless conjunction.
 *
 ...
 * in the last step's return: the quote it is inside (0 for none), the nesting
 * depth, how many further characters it consumed, and whether it ended a
 * selector. Depth back to zero says the scan left the bracket.
 *
 * @internal
...
/**
 * One scan of the bracket opening at `text[from]`: finding where it ends and
 * cutting it into selectors read the same characters. `end` is where the scan
 * stopped, which the caller must check is the `]` it wanted — running off the
 * text or landing on a `)` stops it just the same. Each selector carries its
 * offset into `text`, so a diagnostic points at it rather than the bracket.
 *
 * @param {string} text
 * @param {number} from
 ...
 * index from arrays only, so both are the same guarded child lookup under
 * opposite answers to "is this node an array?".
 *
 * @param {string} spec
...
/**
 * One RFC 9535 slice bound: a negative bound counts from the end, and the
 * result clamps into the window its direction runs in — `[0, len]` walking up,
 * `[-1, len - 1]` walking down, which is what the `up` term shifts both ends
 * of. An absent bound falls back to an end of that same window, and which end
 * swaps when the walk turns around.
 *
 * @param {string} raw
 * @param {number} len
 * @param {number} far 1 for the bound the walk runs towards, 0 for the one it
 *   starts from.
...
let sliceBound = (
  ...
  lo = up - 1,
  hi = len - 1 + up,
  n = raw ? (+raw < 0 ? +raw + len : +raw) : far ^ up ? lo : hi,
...
let sliceSel = (m, step = m[3] ? +m[3] : 1) => ({
  ...
  r: (node, root, ctx, out = /** @type {Loc[]} */ ([])) => {
    ...
    const len = node.value.length,
      up = +(step > 0),
      to = sliceBound(m[2], len, 1, up);
    for (let j = sliceBound(m[1], len, 0, up); (to - j) * step > 0; j += step)
...
// `flatMap` written out. It re-reads the callback's result through the full
// array protocol per node; the two loops measure 1.9x faster on a filter
// segment and 1.3x on a name segment.
/**
 * @param {Loc[]} ns
...
/**
 * One compiled segment plus its entry in the dependency topology. A descendant
 * segment wraps its topology in `["descendant", …]`, is never singular, and
 * expands each incoming node to itself plus its descendants before `each`.
 *
 * Singularity is read off the topology: a name and an index are the
 * two-element tuples, and every other spelling selects more than one node.
 *
 * @param {Seg[]} segs
 * @param {any[]} dep
 * @param {boolean} desc
  ...
  typeof v === "string"
    ...
    : Array.isArray(v)
      ...
      : isObj(v)
        ...
        : NOTHING;
// Null-prototype: a filter names the function, so a normal object would resolve
// `constructor` to Object's.
/** @type {Record<string, Fn>} */
const RFCFN = Object.assign(Object.create(null), {
 ...
 *
 * `span`, never `offset`: the pattern crossed a JSON decode, so no shift lands
 * on the offender, and the literal that carried it is the honest span. `adopt`
 * then gives the span to the budget diagnostics, which have none of their own
 * — relocation never invents one, but a position in the query is ours to know.
 *
...
/**
 * The validity test is a bitwise conjunction: neither half can throw, and the
 * branchless form keeps this function inside the complexity budget.
 *
 * @param {string} key
 * @param {any} value
 */
...
export function query(path, funcs, options) {
  ...
  // caller handed in, so the whitespace it dropped is where coordinates start.
  ...
  fns = funcs;
  meta = { p: /** @type {any[]} */ ([["$"]]), f: /** @type {string[]} */ ([]) };
  const { segs } = segments(path, 1, false, meta.p[0]);
  // One budget slot each: the caller's checked value, the depth default
  // filling an absent one, and an explicit Infinity normalized to the
  // undefined slot `limit()` already reads as unbounded.
  ...
  runner.paths = freeze([
    ...new Map(meta.p.map((x) => /** @type {[string, any]} */ ([JSON.stringify(x), x]))).values(),
  ]);
