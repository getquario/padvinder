...
/**
 * The per-run budget tuple, or null for the no-budget fast path. Slots mirror
 * LIMITS order. Safety by construction: depth defaults to 500, so a deep
...
let budgetTuple = (opts) => {
  const limits = [slot(opts.maxNodes), slot(opts.maxDepth, DEPTH), slot(opts.maxResults)];
  return limits.some((x) => x !== undefined) ? limits : null;
};

// Infinity is the deliberate opt-out spelling: this budget, unbounded.
/** @param {any} value */
let validBudget = (value) => value === Infinity || (Number.isSafeInteger(value) && value >= 0);
/**
 * @param {string} key
 * @param {any} value
 */
