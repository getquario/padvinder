 ...
 */
let kids = (node, ctx) => {
	const value = node.value, depth = node.depth + 1, out = /** @type {Loc[]} */ ([]);
 ...
 */
let all = (node, ctx) => {
	const out = /** @type {Loc[]} */ ([]), stack = /** @type {any[]} */ ([node]), seen = new Set();
	while (stack.length) {
		node = stack.pop();
 ...
 */
let unq = raw => {
	const q = raw[0], end = raw.length - 1;
	let out = '';
	/** @type {() => never} */
		...
		if (c === '\\') {
			...
			if (c === q) out += c;
				...
				if (a >= 0xd800 && a <= 0xdbff) {
					(raw.slice(j + 1, j + 3) === '\\u' && /^[\da-f]{4}$/i.test(raw.slice(j + 3, j + 7))) || bad(); // oxlint-disable-line no-unused-expressions
...
let close = (text, from) => {
	...
	for (; from < text.length; from++) {
		const c = text[from];
		if (q) { if (c === '\\') from++; else if (c === q) q = 0; }
		else if (c === '"' || c === "'") q = c;
		else if (c === '[' || c === '(') d++;
		else if (c === ']' || c === ')') { if (!--d) return c === ']' ? from : err('Missing ] in path'); }
	}
	err('Missing ] in path');
};
...
let split = text => {
	...
	for (let j = 0; j < text.length; j++) {
		...
		else if (c === ',' && !d) { out.push(text.slice(start, j)); start = j + 1; }
	}
...
let selector = (spec, fns, meta) => {
	...
	if (spec[0] === '?') {
		...
		return { run: (n, root, ctx) => kids(n, ctx).filter(c => test(c.value, root, ctx)), topology: ['filter'] };
	}
	...
	if (sliceMatch) {
		...
		return { topology: ['slice', sliceMatch[1] ? +sliceMatch[1] : null, sliceMatch[2] ? +sliceMatch[2] : null, st], run: (n, root, ctx) => {
			if (!Array.isArray(n.value) || !st) return [];
			const len = n.value.length, norm = (/** @type {number} */ x) => (x < 0 ? x + len : x), out = /** @type {Loc[]} */ ([]);
			if (st > 0) {
				...
				const hi = Math.min(Math.max(sliceMatch[1] ? norm(+sliceMatch[1]) : len - 1, -1), len - 1);
	...
	if (q) {
		const k = unq(spec);
		return { run: (n, root, ctx) => Array.isArray(n.value) ? [] : child(n, k, ctx), topology: ['name', k] };
	}
	if (/^-?\d+$/.test(spec)) return { run: (n, root, ctx) => Array.isArray(n.value) ? child(n, spec, ctx) : [], topology: ['index', +spec || 0] };
...
let segments = (path, j, fns, soft, meta, dep) => {
	...
	while (j < path.length) {
		...
		if (path[j] === '[') {
			...
			segs.push({ d: desc, s: sing, f: (ns, root, ctx) => ns.flatMap(n => sels.flatMap(sel => sel.run(n, root, ctx))) });
			j = end + 1;
		} else {
...
let run = (segs, start, root, ctx) => {
	...
	nodes = segs.reduce((acc, s) => s.f(s.d ? acc.flatMap(n => all(n, ctx)) : acc, root, ctx), nodes);
...
let deepEq = (left, right) => {
	...
	while (stack.length) {
		...
		if (x && y && typeof x === 'object' && typeof y === 'object' && Array.isArray(x) === Array.isArray(y)) {
			const keysLeft = Object.keys(x).filter(k => !BLOCK(k)), keysRight = Object.keys(y).filter(k => !BLOCK(k));
...
let cmp = (op, left, right) =>
	op === '==' ? (left === NOTHING || right === NOTHING ? left === right : deepEq(left, right)) :
	op === '!=' ? !cmp('==', left, right) :
	op === '<=' ? cmp('==', left, right) || cmp('<', left, right) :
	op === '>=' ? cmp('==', left, right) || cmp('<', right, left) :
	op === '>' ? cmp('<', right, left) :
	(typeof left === typeof right && (typeof left === 'number' || typeof left === 'string') && left < right); // <
...
let rfcFilter = (source, fns, meta) => {
	...
	const ws = () => { while (/\s/.test(source[k])) k++; };
	/** @param {string} c */
	...
	const literal = () => {
		...
		if (source[k] === '"' || source[k] === "'") {
			...
			j < source.length || fail(); // oxlint-disable-line no-unused-expressions
	...
	const arg = type => {
		...
		if (source[k] === '@' || source[k] === '$') {
			...
			query.s || fail(); // oxlint-disable-line no-unused-expressions
			return (n, r, ctx) => { const ns = query.f(n, r, ctx); return ns.length ? ns[0].value : NOTHING; };
		}
	...
	const asValue = prim => {
		...
		prim.q && prim.q.s || fail(); // oxlint-disable-line no-unused-expressions
		const q = /** @type {NonNullable<Prim['q']>} */ (prim.q);
		return (n, r, ctx) => { const ns = q.f(n, r, ctx); return ns.length ? ns[0].value : NOTHING; };
	};
	...
	const e = or();
	ws();
	k === source.length || fail(); // oxlint-disable-line no-unused-expressions
	return e;
};
