...
- `.oxfmtrc.json` exists for exactly one reason: `test/cts.json` is vendored verbatim from the JSONPath compliance suite (BSD-2, `test/cts.LICENSE`) and re-fetched by `npm run cts:update`. Formatting it would break `fmt:check` on the next update and destroy the ability to diff it against upstream to prove the conformance cases are untampered. It is the only ignore, and the only package with a formatter config.
- Tests use `node:test`: unit and compliance suites live in `test/*.test.js` (`query`, `errors`, `safety`, `compliance`), the type smoke check is `test/types.check.ts`, and browser fixtures live in `test/browser/`.
...
- Bindings are named for readers: `node`/`value`/`depth` not `x`/`n`/`d`, `children` not `cs`, `nodes` not `ns`, `sliceMatch` not `sl`, `keysLeft`/`keysRight` not `kx`/`ky`, `left`/`right` not `a`/`b`. Internal typedef properties too — `Loc.value`/`.depth`, `Sel.run`/`.topology`. Loop counters stay short. Rename with a scope-aware tool, never `sed`: `s` alone named the raw string, the bracket text and the selector spec in three different functions. Watch shorthand properties — renaming the binding in `{ x }` renames the _property_ too.
