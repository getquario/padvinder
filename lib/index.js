 ...
 * bracket closer it landed on, and whether it ended a selector — a comma at
 * the bracket's own level, or that level's closer.
 *
 * @internal
...
let stepBare = (c, d, nest = NEST[c] || 0) =>
  "\"'".includes(c)
    ? [c, d, 0]
    : [0, d + nest, 0, nest < 0 ? c : undefined, !!(+(d == 1) & (+(c == ",") | +(nest < 0)))];
/**
 * @param {string} c
 * @param {string | 0} q
 * @param {number} d
