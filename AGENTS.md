...
`match()`/`search()` compile through treffer with `{ anchors: true }`. `reTest()` keeps a one-entry cache so a document-supplied pattern compiles at most once while filtering a document. Treffer errors and resource-limit failures become false, preserving RFC function behavior.
...
- Treffer is the only runtime dependency. Keep `match()`/`search()` on treffer with `{ anchors: true }`, preserve the one-entry cache, and convert matcher errors to false.
