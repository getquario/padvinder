 ...
 * the nesting depth, how many further characters the step consumed, the
 * bracket closer it landed on, and whether it is a top-level comma.
 *
 * @internal
 * @typedef {[string | 0, number, number, string?, boolean?]} Step
 */
...
let stepBare = (c, d, nest = NEST[c] || 0) =>
  "\"'".includes(c) ? [c, d, 0] : [0, d + nest, 0, nest < 0 ? c : undefined, !!(+(c == ",") & +!d)];
/**
 * @param {string} c
 * @param {string | 0} q
 * @param {number} d
 ...
 * @returns {number}
 */
let close = (text, from, step = /** @type {Step} */ ([0, 0, 0])) => {
 ...
 */
let split = (text, base, step = /** @type {Step} */ ([0, 0, 0])) => {
...
let bracketSeg = (path, j, desc, registry, info, dep) => {
  ...
  // Singularity is already decided by the topology the selectors published:
  // a name and an index are the two-element tuples, and every other spelling —
  // wildcard, filter, slice, and any union of them — is not.
  const sing = +(m.length === 2);
  /** @type {Sel['r']} */
  const each = (n, root, ctx) => sels.flatMap((sel) => sel.r(n, root, ctx));
  return [end + 1, makeSeg(dep, desc, m, sing, each)];
};
/**
 * @param {string} path
 * @param {number} j
...
) => [j + key.length, makeSeg(dep, desc, star ? ["wildcard"] : ["name", key], +!star, each)];
