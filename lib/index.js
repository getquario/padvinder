...
export function query(path, funcs, options) {
  ...
  // One budget slot each, in LIMITS order. maxDepth is the one budget with a
  // default: 500, so a deep untrusted document throws a typed diagnostic
  // rather than overflowing the native stack. An explicit Infinity needs no
  // normalizing: nothing is ever greater than it, so `limit()` reads it as
  // unbounded exactly as it reads an absent budget.
  const limits = [opts.maxNodes, opts.maxDepth ?? 500, opts.maxResults];
  const own = {};
