...
import { adopt, capped, mint, relocate as relocateFault, store } from "waarmerk";
import {
  compile as compileRE,
  isDiagnostic as isREDiagnostic,
  relocate as relocateRE,
} from "treffer";
 ...
 * @import { TrefferErrorCode } from 'treffer'
 ...
 * Every run carries one; an unbounded budget is an undefined slot.
 *
...
/**
 * One compiled selector: `r` executes it against a node, `t` is the
...
/**
 * A relative or absolute query, compiled: `f` runs it to a nodelist and `s`
 * is the RFC 9535 singularity that ValueType position demands.
 *
 * @internal
 * @typedef {{ s: boolean, f: (n: any, r: any, ctx: Ctx) => Loc[] }} Query
 */

/**
 * A parsed filter primary, carrying its RFC declared type and its producer:
 * `t` is 0 for a value, 1 for a query (whose `f` is a {@link Query}, not an
 * `Arg`), 2 for a logical function, 3 for one from the caller's registry.
 * The type is what every position demands against, so a primary keeps the one
 * shape from where it is parsed to where it is used.
 *
 * @internal
 * @typedef {{ t: number, f: any }} Prim
 */
 ...
 * line that throws it rather than shipping — which is how
 * `PADVINDER_MAX_COMPARISONS` got out.
 *
 * The union includes Treffer's, because a pattern literal rejected when the
 * query compiles keeps the code Treffer gave it. Saying so here is what makes
 * `PadvinderDiagnostic.code` true by construction rather than by hand.
 *
 * @type {Store<PadvinderErrorCode | TrefferErrorCode>}
 */
const diags = store("padvinder");
 ...
 * `span` replaces it, for one whose text reached the query through a decode.
 *
 * @type {(diag: unknown, opts?: Relocation) => PadvinderDiagnostic}
 */
 ...
 * it is.
 *
 * `code` names the category of authoring mistake, and defaults to the residue:
 * the two path faults and the filter parser's catch-all are open-ended — "what
 * you wrote is not a query here" — so there is no finite set of categories to
 * put them in. The five that are a single named mistake say so.
 *
 * @type {(m: string, start: number, end: number, code?: PadvinderErrorCode) => never}
 */
const err = (m, start, end, code = "PADVINDER_SYNTAX") =>
  mint(diags, SyntaxError, m, { code, start: base + start, end: base + end });
...
let base = 0;
/**
 * The caller's function registry and the dependency metadata being collected,
 * both fixed for the whole of one compile: `query()` sets them before it parses
 * a character, so no parser below it carries them as a parameter.
 *
 ...
 * the whole literal.
 *
 * One code covers the whole category; `m` is what tells the seven conditions
 * apart, because which of them went wrong is a thing an author reads rather
 * than a thing a consumer branches on.
 *
 * @type {(m: string, from: number, to: number) => never}
 */
const bad = (m, from, to) => err(m, from, to, "PADVINDER_BAD_STRING");
...
/**
 * Every exhausted budget goes through here, for the camelCase name its message
 * needs. The code is checked by the store, not by this signature.
 *
 * @type {(name: string, code: PadvinderErrorCode, max: number, actual: number,
 *   origin?: any) => never}
 */
const cap = (name, code, max, actual, origin) => capped(diags, name, code, max, actual, origin);
/**
 * @param {Ctx} ctx
 * @param {number} key
 * @param {number} actual
...
// Guarded edge into `out` — the one collector every traversal loop shares, and
// the one place a reached node is charged for. Both budgets are charged before
// `obj[key]` is read, so an exhausted budget throws without ever running the
// next sibling's getter.
/**
 * @param {Loc[]} out
 * @param {any} obj
 * @param {any} key
...
let put = (out, obj, key, depth, ctx) => {
  ...
  limit(ctx, 1, depth);
  limit(ctx, 0, ++ctx[3]);
  out.push({ value: obj[key], depth });
};

/** @param {any} obj @returns {string[]} */
let ownKeys = (obj) => Object.keys(obj).filter((key) => !BLOCK(key));
// The two child walks are split by node type rather than by a key list one of
// them has to build: an array index loop allocates nothing, and the blocked
// names are skipped in place instead of through a filtered copy per object.
/**
 * @param {Loc[]} out
 * @param {any[]} value
 * @param {number} depth
 * @param {Ctx} ctx
 */
let indexKids = (out, value, depth, ctx) => {
  for (let j = 0; j < value.length; j++) put(out, value, j, depth, ctx);
};
/**
 * @param {Loc[]} out
 * @param {any} value
 ...
 * @param {Ctx} ctx
 */
let nameKids = (out, value, depth, ctx) => {
  for (const key of Object.keys(value)) if (!BLOCK(key)) put(out, value, key, depth, ctx);
};
/**
 * @param {Loc} node
 * @param {Ctx} ctx
...
let kids = (
  ...
  // oxlint-disable-next-line no-unused-expressions
  isObj(value) &&
    (Array.isArray(value) ? indexKids(out, value, depth, ctx) : nameKids(out, value, depth, ctx));
  return out;
};
 ...
 * all arrives as -1.
 *
 * `noun` is what the two readers disagree about: the escape reader is looking
 * at a `\uXXXX`, the raw reader at a bare code unit. Naming it here keeps the
 * rule in one place while still telling an author which of the two they wrote.
 *
 * @param {number} a The code unit being read.
 * @param {number} low The code unit after it.
 * @param {number} from
 * @param {number} to
 * @param {string} noun What the offending half is, for the message.
 * @returns {boolean} Whether `a` opened a pair, so the reader takes both.
 */
let pairs = (a, low, from, to, noun) => {
  if ((a & 0xfc00) === 0xd800) {
    // oxlint-disable-next-line no-unused-expressions
    (low & 0xfc00) === 0xdc00 || bad("Unpaired high " + noun, from, to);
    return true;
  }
  // oxlint-disable-next-line no-unused-expressions
  (a & 0xfc00) !== 0xdc00 || bad("Lone low " + noun, from, to);
  return false;
};
/**
 * @param {string} raw
...
let unescape = (
  ...
  q,
  at,
  // The five named escapes, as parallel strings: the letter's position in one
  // is the character's position in the other, and a letter that is in neither
  // indexes past the end to `undefined`, exactly as a table miss did.
  ch = ("/\\" + q).includes(raw[j]) ? raw[j] : "\b\f\n\r\t"["bfnrt".indexOf(raw[j])],
) => {
  if (ch) return [ch, 0];
  if (raw[j] === "u") return unescapeU(raw, j, at);
  bad("Unknown escape \\" + raw[j], at + j - 1, at + j + 1);
};
/**
 * Throw for a character that cannot stand unescaped in a literal. Two faults
 * share one test and one span in `stepUnq` — a character below the space, and
 * the delimiter itself — so only the message tells them apart, and choosing it
 * here leaves the reader the branchless conjunction its complexity budget
 * wants.
 *
 * @type {(c: string, q: string, from: number, to: number) => never}
 */
let badRaw = (c, q, from, to) =>
  bad(c === q ? "Unescaped " + q + " in string literal" : "Unescaped control character", from, to);
/**
 * @param {string} raw
 * @param {number} j
 * @param {string} q
...
let stepUnq = (raw, j, q, at) => {
  ...
  const c = raw[j];
  // oxlint-disable-next-line no-unused-expressions
  +(c >= " ") & +(c !== q) || badRaw(c, q, at + j, at + j + 1);
  // A surrogate pair is one scalar value, so it steps two code units at once.
  return pairs(c.charCodeAt(0), raw.charCodeAt(j + 1), at + j, at + j + 1, "surrogate")
    ? [c + raw[j + 1], j + 2]
    : [c, j + 1];
};
 ...
 * @returns {Step}
 */
let stepBare = (c, d, nest = +"[(".includes(c) - +"])".includes(c)) =>
 ...
 * off the text or landing on a `)` stops the scan just the same. Each selector
 * comes back paired with its offset into `text`, so a diagnostic points at the
 * selector rather than at the whole bracket.
 *
 * @param {string} text
 * @param {number} from
 * @returns {{ end: number, specs: [spec: string, at: number][] }}
 */
let brackets = (text, from, step = /** @type {Step} */ ([0, 0, 0]), j = from) => {
  const specs = /** @type {[spec: string, at: number][]} */ ([]);
  let start = from + 1;
  ...
  const add = (at, raw = text.slice(start, at), lead = raw.trimStart()) => {
    specs.push([lead.trimEnd(), start + raw.length - lead.length]);
    start = at + 1;
  };
  for (; j < text.length; j++) {
...
let nameOrIndex = (
  ...
  if (+!quoted & +!/^-?\d+$/.test(spec))
    err("Bad selector [" + spec + "]", at, at + spec.length, "PADVINDER_BAD_SELECTOR");
  /** @type {Sel['r']} */
...
let selector = (
  ...
  if (spec[0] === "?") {
    const test = rfcFilter(spec.slice(1), at + 1);
    /** @type {Sel['r']} */
 ...
 * that is itself the selector. Anything else starts no segment — which soft
 * mode reports as null so the caller can stop there, and hard mode refuses.
 *
 * @param {string} path
 * @param {number} j
 * @param {boolean} soft
 ...
 */
let segPrefix = (path, j, soft, desc = path.startsWith("..", j)) =>
  path[j] === "."
    ? [desc, j + 1 + +desc]
    : path[j] === "["
      ? [false, j]
      : soft
        ? null
        : err('Unexpected "' + path[j] + '" in path', j, j + 1);
// Apply one step to every node and collect what it selected, in order. This is
// what `flatMap` means, written out: `flatMap` re-reads the callback's result
// through the full array protocol per node, and a segment runs once per node in
// the list. Measured on the comparison fixture, writing the two loops out is
// worth 1.9x on a filter segment and 1.3x on a name segment.
/**
 * @param {Loc[]} ns
 * @param {(n: Loc) => Loc[]} step
 * @returns {Loc[]}
 */
let expand = (ns, step) => {
  const out = /** @type {Loc[]} */ ([]);
 ...
 *
 * The segment lands in `segs` the way its topology lands in `dep`, so neither
 * kind of caller carries a compiled segment back up to be collected.
 *
 * @param {Seg[]} segs
 * @param {any[]} dep
 * @param {boolean} desc
 ...
 */
let makeSeg = (segs, dep, desc, topo, each) => {
  dep.push(desc ? ["descendant", topo] : topo);
  segs.push({
    s: !!(+!desc & +(topo.length === 2)),
    f: (ns, root, ctx) =>
      expand(desc ? expand(ns, (n) => all(n, ctx)) : ns, (n) => each(n, root, ctx)),
  });
};
/**
 * @param {string} path
 ...
 * @param {any[]} dep
 * @param {Seg[]} segs
 * @returns {number} Where the segment ended.
 */
let bracketSeg = (path, j, desc, dep, segs) => {
  const { end, specs } = brackets(path, j);
  // oxlint-disable-next-line no-unused-expressions
  path[end] === "]" || err("Missing ] in path", j, j + 1, "PADVINDER_UNCLOSED_BRACKET");
  const sels = specs.map((spec) => selector(spec[0], spec[1]));
  const m = specs.length === 1 ? sels[0].t : ["union", ...sels.map((s) => s.t)];
  /** @type {Sel['r']} */
  const each = (n, root, ctx) => sels.flatMap((sel) => sel.r(n, root, ctx));
  return (makeSeg(segs, dep, desc, m, each), end + 1);
};
/**
 * @param {string} path
 * @param {number} j
...
let identSeg = (
  ...
  (makeSeg(segs, dep, desc, star ? ["wildcard"] : ["name", key], each), j + key.length);
 ...
 */
let segments = (path, j, soft, dep) => {
  const segs = /** @type {Seg[]} */ ([]);
  while (j < path.length) {
    const back = j;
    j = skip(path, j);
    const prefix = segPrefix(path, j, soft);
    if (!prefix) return { segs, j: back };
    // The two segment kinds take the same arguments and return the same thing,
    // so the bracket test picks the parser rather than the whole call.
    j = (path[prefix[1]] === "[" ? bracketSeg : identSeg)(path, prefix[1], prefix[0], dep, segs);
  }
  return { segs, j };
};
...
let run = (segs, start, root, ctx) => {
  // The start node is reached, so it is charged like any other; the depth
  // budget has nothing to say about depth 0, which is never over a budget
  // `checkBudget` has already forced non-negative.
  limit(ctx, 0, ++ctx[3]);
  const nodes = segs.reduce((acc, s) => s.f(acc, root, ctx), [{ value: start, depth: 0 }]);
...
const reFn = (full) => ({
  ...
  c: (args, last, end, p = /** @type {any} */ (args[1]).lit) => {
    if (typeof p !== "string") return;
    try {
      compileRE(p, RE_OPTS);
    } catch (e) {
      if (!isREDiagnostic(e)) throw e;
      throw located(e, base + last, base + end);
    }
  },
  f:
    ([a, b]) =>
 ...
 *   c?: (args: Arg[], last: number, end: number) => void }} Fn
 */
/**
...
const queryExpr = (abs = source[k++] === "$", dep = [abs ? "$" : "@"]) => {
  meta.p.push(dep);
  const { segs, j } = segments(source, k, !0, dep);
  k = j;
  return {
    s: segs.every((s) => s.s),
...
let singularVal = (q) =>
  // oxlint-disable-next-line no-unused-expressions
  (q.s || fail(), (n, r, ctx, ns = q.f(n, r, ctx)) => (ns.length ? ns[0].value : NOTHING));
/** @returns {Arg} */
...
const arg = (type) => {
  ...
  // NodesType wants the whole nodelist as values; every other position wants
  // the one value a singular query stands for.
  const q = queryExpr();
  return type === 1 ? (n, r, ctx) => q.f(n, r, ctx).map((x) => x.value) : singularVal(q);
};
let restArgs = () => {
  const args = /** @type {Arg[]} */ ([]);
  do args.push(arg(0));
...
let userArgs = (name, from, f = fns[name]) => {
  // oxlint-disable-next-line no-unused-expressions
  Object.hasOwn(fns, name) ||
    err(name + " is not a function", from, from + name.length, "PADVINDER_UNKNOWN_FUNCTION");
  // oxlint-disable-next-line no-unused-expressions
  meta.f.includes(name) || meta.f.push(name);
  ws();
  const args = eat(")") ? [] : restArgs();
  /** @type {Arg} */
  const call = (n, r, ctx) => f(...args.map((a) => a(n, r, ctx)));
  return { t: 3, f: call };
};
/** @returns {Prim} */
const funcExpr = (
  ...
  // `from` has done its job by here, so it carries where the argument being
  // parsed began — which, once the map is done, is where the last one began.
  // A compile-time check only ever looks at that one.
  // oxlint-disable-next-line no-unused-vars
  const args = spec.a.map((t, x) => (x && (ws(), expect(",")), ws(), (from = k), arg(t)));
  // oxlint-disable-next-line no-unused-expressions
  spec.c && spec.c(args, from, k);
  ws();
  expect(")");
...
/**
 * A Treffer diagnostic, in this query's coordinates and this module's store.
 *
 * Treffer is the authority on what is wrong with the pattern, so its class,
 * its code and its `limit`/`actual` all survive — a host that wants to know
 * the pattern was over budget rather than malformed can still tell. What does
 * not survive is Treffer's span: the pattern reached it through JSON
 * unescaping, where an escape slides every later offset, so there is no shift
 * that lands on the offending character. The literal that carried the pattern
 * is the honest span, and `span` replaces rather than shifts for exactly this.
 *
 * `adopt` then does two things at once. It takes the copy into this module's
 * store, so the diagnostic passes both guards as thrown and an embedder of
 * padvinder sees it as ours. And it carries the span for the diagnostics that
 * had none: relocation never invents a span, and this is not relocation
 * inventing one — an over-budget pattern has no position *in the pattern*,
 * which is Treffer's coordinate system, but it has one in the query, which is
 * ours to know. Documented on `PadvinderDiagnostic.start`. For the located
 * ones it redefines `start` and `end` to the values `span` already gave them,
 * which is the one redefinition a non-configurable property allows.
 *
 * @type {(e: unknown, start: number, end: number) => PadvinderDiagnostic}
 */
let located = (e, start, end) =>
  adopt(diags, relocateRE(e, { span: [start, end] }), { start, end });

// A comparable/test primary: query, literal, or function call. A function call
// already carries its RFC type, so it is a primary as it stands.
/** @returns {Prim} */
const primary = () => {
  ws();
  if ("@$".includes(source[k])) return { t: 1, f: queryExpr() };
  const lit = literal();
  return lit ? { t: 0, f: lit } : funcExpr();
};
// ValueType position: literals, value functions, and singular queries only —
// a query has to be demanded singular first, and a logical function has no
// value at all.
/**
 * @param {Prim} p
 * @returns {Arg}
 */
const asValue = (p) => (p.t === 1 ? singularVal(p.f) : p.t === 2 ? fail() : p.f);
...
// Test position: a query is an existence test, a logical or registered
// function is its own test, and a plain value is not a test at all.
/**
 * @param {Prim} p
 * @returns {Arg}
 */
let asTest = (p, f = p.f) =>
  p.t === 1 ? (n, r, ctx) => f.f(n, r, ctx).length > 0 : p.t ? f : fail();
/**
 * @param {boolean} neg
 * @param {Arg} t
 * @returns {Arg}
 ...
 * @returns {Test}
 */
let rfcFilter = (src, at, prev = /** @type {[string, number, number]} */ ([source, k, base])) => {
  source = src;
  k = 0;
  base += at;
  try {
...
let checkOpts = (options) => {
  for (const key of Object.keys(options)) {
    if (!LIMITS.includes(key)) mint(diags, TypeError, 'Unknown option "' + key + '"');
...
let checkBudget = (key, value) => {
  if (typeof value !== "number") mint(diags, TypeError, key + " must be a number");
  if (!(+(value === Infinity) | (+Number.isSafeInteger(value) & +(value >= 0))))
    mint(diags, RangeError, key + " must be a non-negative safe integer or Infinity");
};
...
export function query(path, funcs, options) {
  ...
  // oxlint-disable-next-line no-unused-expressions
  path[0] === "$" ||
    err("Path must start with $", 0, Math.min(1, path.length), "PADVINDER_MISSING_ROOT");
  const info = { p: /** @type {any[]} */ ([["$"]]), f: /** @type {string[]} */ ([]) };
  fns = funcs;
  meta = info;
  const { segs } = segments(path, 1, false, info.p[0]);
  // One budget slot each: the caller's checked value, the depth default filling
  // an absent one, and an explicit Infinity opting out — normalized to the
  // undefined slot `limit()` already reads as unbounded. Safety by
  // construction: depth defaults to 500, so a deep untrusted document throws a
  // typed diagnostic instead of overflowing the native stack.
  const limits = LIMITS.map((key, i) => {
    // Slot 1 is maxDepth, the one budget with a default: 500, so a deep
    // untrusted document throws a typed diagnostic rather than overflowing
    // the native stack.
    const v = opts[key] ?? (i === 1 ? 500 : undefined);
    return v === Infinity ? undefined : v;
  });
  const own = {};
  const runner = (/** @type {any} */ data, ctx = [...limits, 0, own]) =>
  ...
  runner.isDiagnostic = (/** @type {any} */ e) => diags.origin(e) === own;
  return runner;
}
