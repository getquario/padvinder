...
let base = 0;
/**
 * The string literal currently being unescaped, in the coordinates of whatever
 * text its caller is parsing. `unq` sets it from the offset it is handed; the
 * frames below it walk an index into the literal alone, which is why the span
 * they fault with is the literal rather than the character inside it.
 *
 * @type {[number, number]}
 */
let lit = [0, 0];
...
/** @type {() => never} */
const bad = () => err("Invalid string literal", lit[0], lit[1]);
...
/**
 * @param {string} raw
 * @param {number} at Offset of `raw` within the text being parsed, for `lit`.
 * @returns {string}
 */
let unq = (raw, at, q = raw[0], end = raw.length - 1) => {
  lit = [at, at + raw.length];
  let out = "",
    j = 1;
  while (j < end) {
