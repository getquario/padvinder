...
- `test/browser/harness.js` rewrites the bare `treffer` and `waarmerk` imports to paths it serves, in `lib/index.js` and in the dependencies themselves, because a browser cannot resolve a bare specifier. Resolve each through `import.meta.resolve`, not a hardcoded path.
