...
export let relocate = (diag, { prefix = "", offset = 0 } = {}) => {
  ...
  let d = /** @type {any} */ (diag),
    props = DESCS(d),
    // A native constructor's `prototype` is non-writable and non-configurable,
    // so reading it off the table entry here is the same value the table was
    // built from — the lookup is as fixed as the list of classes it walks.
    copy = (KINDS.find((Kind) => Kind.prototype === PROTO(d)) || Error)(prefix + d.message);
...
const reFn = (full) => ({
  ...
  c: (args, last, end, p = /** @type {any} */ (args[1]).lit) => {
    ...
    try {
      ...
      if (!isREDiagnostic(e)) throw e;
      err(/** @type {Error} */ (e).message, last, end);
    }
  },
  f:
    ([a, b]) =>
